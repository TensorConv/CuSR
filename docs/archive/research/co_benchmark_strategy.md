...
> **对齐说明 (2026-06-21).** 贡献陈述以 [`contributions.md`](contributions.md) 为准(三条:① 系统 kernel ② 选题判据 ③ 实证发现)。本文件是**推理留痕**,冲突处以 contributions.md 为准。三处更新:
> 1. §1a 的 "systems + honesty 单一头条" = 旧的 2b 立场;现为**三条并列贡献**,判据/发现已从"instrument"升为与 kernel 并列(见 `co_benchmark_contribution_plan.md`)。
> 2. **诚实性不是并列支柱/轴**——Kronberger 2022 (2209.00942) 占该轴;诚实性只当贡献①的正确性特性(本文 §2b-6 已承认机制不新,与此一致)。
> 3. Kozax = **de Vries 2025**(JAX/vmap、非 CUDA、一阶 AD + GA,无 LM),非 "de Wolff"(§4 表已修)。
...
| Kozax (de Vries, Keemink & van Gerven 2025, GECCO Companion) | arXiv 2502.03047 (verified 2026-06-21: JAX/vmap, NOT custom CUDA; CO = first-order AD-gradient + a simple GA, no LM) | GPU first-order gradient CO per-individual over evolved trees | Ours is **second-order LM/NLS with explicit per-tree Jacobians in a custom CUDA kernel**, not first-order GD in JAX | Refutes "no GPU SR does CO" — pre-rebut "why not Gpufit?" (heterogeneous trees, not one fixed model) | Claim only the LM-Jacobian slice. **MEDIUM** (restricted existential) |
