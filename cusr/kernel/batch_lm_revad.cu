...
#include "revad_interp.cuh" // v5: reverse-mode AD — rev_jacobian_kernel + MAX_NODES (drop-in J)
#include "loader.h"
...
int main(int argc, char **argv) {
    ...
    // v5 reverse-AD: per-point tape is sized MAX_NODES (compile-time). Assert the corpus
    // fits (mirror of the max_stack check) so a too-deep tree can't silently overflow d1/d2.
    {
        int max_nn = 0;
        for (int mm = 0; mm < h->M_prob; mm++)
            if (pop.metas[mm].n_nodes > max_nn) max_nn = pop.metas[mm].n_nodes;
        if (max_nn > MAX_NODES) {
            fprintf(stderr, "[batch-lm] max n_nodes=%d > MAX_NODES=%d (reverse-AD tape, compile-time). 增 MAX_NODES 重编.\n",
                    max_nn, MAX_NODES);
            free_pop_data(&pop); return 2;
        }
    }
    ...
    for (int it = 0; it < max_iter; it++) {
        ...
        // A. Reverse-mode AD Jacobian (v5 drop-in, 替 ad_jacobian_kernel / fd_jacobian)。one warp
        // owns a tree, 32 lane 跨 N; 一次 point-pass (前向记 tape + 反向扫描) 即产全部 K 列,
        // 写精确 ∂y_i/∂c_j 直入 d_J (同一 layout)。无 eps, 无 y_base (AD 精确), K-flat。
