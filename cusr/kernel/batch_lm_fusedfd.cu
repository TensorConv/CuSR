...
int main(int argc, char **argv) {
    ...
    // FD 扰动: 相对步长, 实际 h 在 fd_jacobian_fused_kernel 内按 |c| 计算.
    // 绝对 eps 在常数量级远离 1 时毁掉 Jacobian, 见 tests/test_fixture_scale.py
    // 与 batch_lm.cu 同位置注释 (eps_rel=1e-3 经 parity 对拍选定).
    const float eps_fd = 1e-3f;   // 语义: eps_rel (相对步长系数)
