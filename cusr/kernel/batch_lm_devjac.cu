...
// Compile-time bounds. 跑前 runtime guard.
#define MAX_STACK 64
#define MAX_K     32
...
// FD Jacobian column k, assembled ON DEVICE — no host round-trip.
// J[m,k,i] = (yp[m,i] - yb[m,i]) / eps. __fdiv_rn forces IEEE round-to-nearest
// division, so this is bit-identical to the host float divide despite --use_fast_math.
// Divisor is the per-tree actual FD step heps[m] (relative-eps fix, 2026-06-11).
// For k >= K[m] or finished trees yp == yb, so this writes 0 (heps init 1.0, 永不为 0);
// build_jtj never reads those columns, so the assembled JtJ/JtR match the host version exactly.
__global__ void jac_diff_kernel(
    const float *yp, const float *yb, int total, int N, int K_max, int k,
    const float *heps, float *J)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= total) return;
    int m = idx / N;
    int i = idx - m * N;
    J[(size_t)m * K_max * N + (size_t)k * N + i] = __fdiv_rn(yp[idx] - yb[idx], heps[m]);
}
...
int main(int argc, char **argv) {
    ...
    float *d_heps;
    CUDA_CHECK(cudaMalloc(&d_heps, M_prob*sizeof(float)));   // 当前列每棵树的实际 FD 步长
    ...
    // FD 扰动: 相对步长 h = eps_rel*|c| (c==0 退化为 eps_rel), 除数取 fp32 实际
    // 可表示步长. 绝对 eps 在常数量级远离 1 时毁掉 Jacobian, 见
    // tests/test_fixture_scale.py 与 batch_lm.cu 同位置注释 (eps_rel=1e-3 经 parity 对拍选定).
    ...
    for (int m = 0; m < M_prob; m++) h_heps[m] = 1.0f;         // 非活跃槽位除以 1 → 仍写 0, 保持 jac_diff 注释里的不变量
    ...
    for (int it = 0; it < max_iter; it++) {
        ...
        // A. d_y is the baseline (init at start; G refreshes after each iter).
        //    It stays on the device — no copy back to host (this is option A).

        // B. FD Jacobian — assembled ON DEVICE, column by column, straight into d_J.
        for (int k = 0; k < K_max; k++) {
            ...
            CUDA_CHECK(cudaMemcpy(d_heps, h_heps, M_prob*sizeof(float), cudaMemcpyHostToDevice));
            eval_kernel_batched<<<grid, block>>>(d_nt, d_nv, d_ci, d_metas, M_prob, d_xs, n_vars, N, d_call, d_yp);
            CUDA_CHECK(cudaGetLastError());
            // (yp - yb)/h written straight into d_J[:,k,:] on the device — this replaces
            // the D->H copy of yp, the host differencing loop, and the H->D upload of J.
            jac_diff_kernel<<<(M_prob*N+127)/128, 128>>>(d_yp, d_y, M_prob*N, N, K_max, k, d_heps, d_J);
            CUDA_CHECK(cudaGetLastError());
        }
        CUDA_CHECK(cudaMemcpy(d_call, h_c, total_c*sizeof(float), cudaMemcpyHostToDevice));
    ...
    cudaFree(d_J); cudaFree(d_JtJ); cudaFree(d_JtR); cudaFree(d_delta); cudaFree(d_heps);
